...
class CommunicationAgent extends BaseAgent {
  ...
  // Method to facilitate communication between different stakeholders
  async facilitateStakeholderCommunication(request, senderInfo, recipientInfo) {
    const prompt = `
      Facilitate communication between hackathon stakeholders:
      
      Request: ${JSON.stringify(request)}
      Sender: ${JSON.stringify(senderInfo)}
      Recipient: ${JSON.stringify(recipientInfo)}
      
      Based on the request and the roles of the sender and recipient:
      1. Formulate an appropriate message that clearly conveys the sender's request/information
      2. Adapt the language and tone to maintain professionalism and effectiveness
      3. Structure the communication to maximize the likelihood of a positive response
      4. Include any necessary context or background information
      5. Suggest appropriate follow-up actions or timelines
      
      The message should facilitate productive communication while respecting boundaries
      between different stakeholder groups (e.g., participants, sponsors, organizers).
    `;
